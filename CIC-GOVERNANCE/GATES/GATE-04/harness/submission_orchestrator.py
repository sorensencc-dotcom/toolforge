"""Fail-closed Gate-04 R2 submission orchestration."""
...
def validate(report_path: Path, actor: str) -> tuple[dict, list[str]]:
    ...
    if report.get("gate_id") != "GATE-04" or report.get("report_id") != "CIC-TEST-REPORT-GATE-04-R2":
    ...
    if "FILL_IN" in report_path.read_text(encoding="utf-8"):
...
def submit(report_path: Path, actor: str, amendment_id: str, confirmation_path: Path) -> dict:
    ...
    try:
        ...
        ingested = lineage.append(
            ...
            report_hash=_hash(artifact), gate_id="GATE-04"
        )
        ...
        amendment = {
            ...
            "title": "GATE-04 closure - Cross-Process Lineage Locking",
            "status": "PROPOSED",
            "submitted_date": submitted_at[:10],
            "ratified_date": None,
            "submitted_by": actor,
            "ratified_by": None,
            "affected_sections": ["CIC-GATE-SPEC-001 section 6", "MANIFEST/gate-registry.json"],
            "change_summary": "Propose GATE-04 closure after process-level R2 passed 8/8.",
