...
AID='AMD-v2.4.0-GATE-05-CLOSED'; GATE='GATE-05'; REPORT='CIC-TEST-REPORT-GATE-05-R2'; ACTOR='ACT-20260714-0001'
def run():
 c=json.loads((ROOT/'confirmation/GATE-05-R2-CONFIRMATION.json').read_text()); a=json.loads((ROOT/f'AMENDMENTS/{AID}.draft.json').read_text()); g=json.loads((ROOT/'MANIFEST/gate-registry.json').read_text()); reg=json.loads((ROOT/'AMENDMENTS/amendment-registry.json').read_text())
 ...
 a.update(status='RATIFIED',ratified_date=now[:10],ratified_by='Tier 1 (Chris)',linked_lineage_id=closure['lineage_id'],decision_note='Tier 1 ratified Gate-05 closure on 2026-07-14. Runtime activation remains separate.')
