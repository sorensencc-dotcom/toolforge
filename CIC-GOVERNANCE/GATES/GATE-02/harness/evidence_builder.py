"""Build canonical Gate-02 R2 report from deterministic driver evidence."""
...
def build(driver_result: Path, actor: str, output: Path) -> dict:
    ...
    if set(definitions) != set(observed):
        raise ValueError("driver evidence does not contain exactly TC-02-001 through TC-02-008")
    ...
    report = {
        "report_id": "CIC-TEST-REPORT-GATE-02-R2",
        "supersedes": "CIC-TEST-REPORT-GATE-02",
        "gate_id": "GATE-02",
        "gate_name": "Publication and Retries",
        "spec_version": "2.4.0",
        "gate_closure_spec_version": "1.0.3-candidate.1",
        "gate_closure_spec_ref": "CIC-GATE-SPEC-001 section 4",
        "executed_by": actor,
        ...
        "_amendment_target": "AMD-v2.4.0-GATE-02-CLOSED",
        "_minimum_passing_threshold": "8/8 deterministic cases",
        "_submission_ready": bool(
            actor_row
            and actor_row["state"] == "ACTIVE"
            and passed == len(test_cases)
            and result["environment"]["filesystem"] == "NTFS"
        ),
    }
