...
**Phase 0.9** is foundational build infrastructure for CIC OS. It sits in **Phase 0 Infrastructure Layer**, before all feature phases.
...
- Standardizes entire build pipeline (images, directory layout, CI templates)
...
### Positioning
- **Not blocker** — runs in parallel with all active phases
...
- When planning Phase 24 or Phase 4.x work, assume TheFoundry is available as hard dependency by week 2
- When onboarding new developers, point them to TheFoundry dev workflow
- When debugging build failures, check "is this inside container?" first
- When refining Phase 0.9 scope, refer back to "locked" vs "open for refinement" distinction
