...
**Problem Solved:** User reported "50+ approval clicks before lunch" because policy violations were not caught at commit time. Approval system had to handle violations after the fact.
...
2. **Zone Ownership** — each file has a primary owner (zone path); commit author must match
...
- **Zero policy bypasses** — physically cannot create a commit that violates rules
...
**Why:** [[skills-policy-agent-requirement]] — enforcement of shared library adoption with automated governance preventing ad-hoc violations. PolicyValidator is the "self-correcting" approval agent that runs in real-time.
...
See [[phase-7-7-confidence-model]] and [[phase-7-8-drift-calculator]] for related work on deterministic confidence scoring and drift detection. PolicyValidator is the **prevention** layer; those are the **detection** layers.
