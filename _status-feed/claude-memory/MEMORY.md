...
## 2026-08-27
- [Session Wrap: Wiki-QA GitHub-DOM Scoping](session-wrap-2026-08-27-wiki-qa-github-dom.md) — closed the prior entry's open items: PR #6 merged (d2d5754), PR #12 merged (457fcec) scoping the audit to `.markdown-body` on GitHub wiki pages. GOVERNANCE diagram still fails — real content bug (`GOVERNANCE.md:7` `](wiki/…png)` path), fix is in sync-github-wiki.mjs, not the audit.
## 2026-08-17
...
- [Feedback: Codex Scope Creep + Autopush (sigil-repo)](feedback_codex_scope_creep_autopush_sigil.md) — recurrence confirmed, squashed unreviewed commits keep landing on main; check full commit range not just the stated commit; needs PR-per-task gate or dedicated audit budget.
## 2026-08-16
