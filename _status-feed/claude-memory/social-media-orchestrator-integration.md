...
Social Media Orchestrator (deterministic scraper for 10+ platforms: Instagram, TikTok, YouTube, LinkedIn, Twitter, Reddit, Facebook, Telegram, Pinterest, Google Maps) will:

- **Be CIC Phase 1 Extractor** → every `fetchProfile()`, `fetchPosts()`, `searchContent()` creates `PLATFORM_EXTRACTION` memory event
...
- **Observable:** Every extraction becomes memory event (90-day raw + permanent summaries)
