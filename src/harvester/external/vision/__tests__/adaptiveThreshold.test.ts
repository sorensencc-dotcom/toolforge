...
describe("AdaptiveThreshold", () => {
  ...
  describe("initialization and constructor validation", () => {
    ...
    it("throws error when minBound >= maxBound", () => {
      expect(() => new AdaptiveThreshold(0.72, 0.9, 0.5)).toThrow(
        "minBound must be strictly less than maxBound",
      );
      expect(() => new AdaptiveThreshold(0.72, 0.8, 0.8)).toThrow(
        "minBound must be strictly less than maxBound",
      );
    });
    ...
    it("throws error when minBound or maxBound are NaN or nonnumeric", () => {
      ...
      expect(
        () => new AdaptiveThreshold(0.72, 0.5, "invalid" as any),
      ).toThrow();
    });
  ...
  describe("bounds enforcement", () => {
    it("exact boundary: accepts initial 0.50 and 0.95 without alteration", () => {
      const tMin = new AdaptiveThreshold(0.5, 0.5, 0.95);
      expect(tMin.get()).toBe(0.5);

      const tMax = new AdaptiveThreshold(0.95, 0.5, 0.95);
      expect(tMax.get()).toBe(0.95);
    });

    it("clamps values below 0.50 to 0.50", () => {
      const tLow = new AdaptiveThreshold(0.4, 0.5, 0.95);
      expect(tLow.get()).toBe(0.5);

      for (let i = 0; i < 25; i++) {
        threshold.update(0.05, 0.05, 0.0);
      }
      expect(threshold.get()).toBe(0.5);
    });

    it("clamps values above 0.95 to 0.95", () => {
      const tHigh = new AdaptiveThreshold(1.05, 0.5, 0.95);
  ...
  describe("determinism", () => {
    it("produces same result for same inputs", () => {
      ...
      t1.update(0.8, 0.75, 0.1);
      t2.update(0.8, 0.75, 0.1);
