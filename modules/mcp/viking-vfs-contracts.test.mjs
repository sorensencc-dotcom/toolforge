...
const error = { jsonrpc: '2.0', id: 1, error: { code: -32602, message: 'missing', data: { viking_code: 'RESOURCE_NOT_FOUND' } } };
...
test('rejects malformed requests and invalid method parameters', () => {
  const cases = [
    ...
    [request('viking/readBatch', { items: [] }), '$.params.items'],
    [request('resources/list', { offset: 0 }), '$.params.offset'],
...
test('accepts error envelopes and rejects malformed or ambiguous envelopes', () => {
  ...
  assert.throws(() => validateResponse({ ...error, error: { code: 'X', message: 'bad' } }), /finite number/);
});
