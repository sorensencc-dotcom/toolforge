...
export function validateReport(report: any): asserts report is VikingReport {
  ...
  const { tokens_loaded, tokens_saved_vs_L2, percent_reduction } = report.token_delta;
  ...
  if (typeof percent_reduction !== 'number' || percent_reduction < 0 || percent_reduction > 100) {
    throw new Error('percent_reduction must be a number between 0 and 100');
  }

  // B. Validate Cache Effect
  const allowedCacheEffects = ['preserved', 'extended', 'invalidated', 'fragmented'];
  if (!allowedCacheEffects.includes(report.cache_effect)) {
    throw new Error(`Invalid cache_effect: ${report.cache_effect}. Must be one of ${allowedCacheEffects.join(', ')}`);
  }
  ...
  if (!allowedRiskLevels.includes(risk_level)) {
    throw new Error(`Invalid risk_level: ${risk_level}. Must be one of ${allowedRiskLevels.join(', ')}`);
  }

  // E. Validate Model-Tier Suitability
  const allowedSuitability = ['optimal', 'acceptable', 'suboptimal', 'degraded'];
  if (!allowedSuitability.includes(report.model_suitability)) {
    throw new Error(`Invalid model_suitability: ${report.model_suitability}. Must be one of ${allowedSuitability.join(', ')}`);
  }
  ...
  if (typeof rta !== 'number' || !Number.isInteger(rta) || rta < 0 || rta > 3) {
    throw new Error('roundtrip_avoidance_score must be an integer between 0 and 3');
  }
}
...
describe('Viking VFS Telemetry and Reporting Contract Validation', () => {
  ...
  it('should fail validation if tokens_loaded is a negative number', () => {
    ...
    assert.throws(() => validateReport(malformed), /tokens_loaded must be a non-negative number/);
  });

  it('should fail validation if percent_reduction is out of percentage boundaries', () => {
    const malformed = {
      ...validReportFixture,
      token_delta: { ...validReportFixture.token_delta, percent_reduction: 105.2 }
    };
    assert.throws(() => validateReport(malformed), /percent_reduction must be a number between 0 and 100/);
  });
  ...
  it('should fail validation on an invalid cache_effect choice', () => {
    ...
    assert.throws(() => validateReport(malformed), /Invalid cache_effect: shattered/);
  });
  ...
  it('should fail validation if context usage percentage is out of range', () => {
    ...
    assert.throws(() => validateReport(malformed), /usage_pct must be a percentage between 0 and 100/);
  });
  ...
  it('should fail validation on invalid risk_level keys', () => {
    ...
    assert.throws(() => validateReport(malformed), /Invalid risk_level: extreme/);
  });
  ...
  it('should fail validation if roundtrip_avoidance_score is outside the 0-3 range', () => {
    ...
    assert.throws(() => validateReport(malformed), /roundtrip_avoidance_score must be an integer between 0 and 3/);
  });
  ...
  it('should fail validation if roundtrip_avoidance_score is not a whole integer', () => {
    ...
    assert.throws(() => validateReport(malformed), /roundtrip_avoidance_score must be an integer/);
  });
  ...
  it('should intercept validation failures and abort with a fail-closed -32005 error code', () => {
    ...
    assert.match(response.error.message, /VIKING_REPORT_INVALID: Invalid model_suitability: unsupported_model/);
