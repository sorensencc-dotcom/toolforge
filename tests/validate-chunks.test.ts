...
# Content`;
    ...
    it("fails when frontmatter block is missing", () => {
      const result = validateFileProvenance(
        "/fake/path.md",
        "# No frontmatter",
      );
      ...
      for (const field of REQUIRED_PROVENANCE_FIELDS) {
        ...
        assert.ok(
          result.errors.some((e) =>
            e.includes(`Missing mandatory provenance field: '${field}'`),
          ),
    ...
    it("validates verification_status enum constraints", () => {
      ...
      for (const status of allowed) {
        ...
        assert.equal(
          result.valid,
          true,
          `Expected valid status for '${status}'`,
        );
      }

      const invalidStatusContent = validContent.replace(
        "verification_status: verified",
      ...
      assert.equal(result.valid, false);
      assert.ok(
        result.errors.some((e) => e.includes("Invalid verification_status")),
      );
    });
  });
...
# Content`;
    ...
    it("passes cleanly on directory containing valid research notes", () => {
      ...
      fs.writeFileSync(
        path.join(researchDir, "note-1.md"),
        validMarkdown,
        "utf8",
      );
      fs.writeFileSync(
        path.join(researchDir, "note-2.md"),
        validMarkdown,
        "utf8",
      );
    ...
    it("fails and reports errors when malformed notes exist", () => {
      ...
      fs.mkdirSync(researchDir, { recursive: true });
      fs.writeFileSync(
        path.join(researchDir, "valid.md"),
        validMarkdown,
        "utf8",
      );
      fs.writeFileSync(
        path.join(researchDir, "invalid.md"),
        invalidMarkdown,
        "utf8",
      );
    ...
    it("ignores non-markdown files and excluded directories (node_modules, .git, .nlm_pack)", () => {
      ...
      fs.writeFileSync(
        path.join(ignoredModules, "bad.md"),
        invalidMarkdown,
        "utf8",
      );
      fs.writeFileSync(
        path.join(ignoredGit, "bad.md"),
        invalidMarkdown,
        "utf8",
      );
      fs.writeFileSync(
        path.join(ignoredNlm, "bad.md"),
        invalidMarkdown,
        "utf8",
      );
      fs.writeFileSync(
        path.join(researchDir, "notes.txt"),
        "Not markdown",
        "utf8",
      );
      fs.writeFileSync(
        path.join(researchDir, "good.md"),
        validMarkdown,
        "utf8",
      );
...
# Content`;
    ...
    it("exits with code 1 when malformed files exist (blocking pre-commit)", () => {
      ...
      fs.writeFileSync(
        path.join(invalidDir, "bad-note.md"),
        invalidMarkdown,
        "utf8",
      );
