...
**2a. Real gaps found only by Task 5's live verification, not by mocked tests:** Task 3/4's `defaultExec` design (`spawnSync(CLAUDE_BIN, args)` with the prompt as a `-p` argv value) was plausible and typechecked, but failed against the real `claude` binary in three escalating ways only a live run surfaces:

1. Windows `spawnSync` can't exec a `.cmd` wrapper directly — `EINVAL`. Fixed by routing through `cmd.exe /d /s /c` with an args array (not `shell: true`, which was tested and confirmed to mangle args — splits on spaces, breaks on `&`, matching Node's own DEP0190 deprecation warning).
2. Even routed through `cmd.exe`, a multi-line prompt passed as a `-p` argv value got corrupted — the model received something malformed enough that it ignored `--output-format json` entirely and returned prose. Root cause: `cmd.exe`'s command-line parsing can't be trusted with embedded newlines in an argv element, regardless of quoting. Fixed by moving the entire prompt *and* source content into `stdin`, leaving `argv` as a fixed, short, single-token flag list (`--print --output-format json --model sonnet`) that never contains prompt content at all. This incidentally closes a shell-injection surface too: operator-supplied source titles no longer touch argv.
3. Once JSON actually came back, the model wrapped it in a ` ```json ` markdown fence despite the prompt's explicit "no prose outside the JSON" instruction — common LLM behavior the plan hadn't accounted for. Fixed with a `stripCodeFence()` helper before `JSON.parse`.

All three fixes are in `src/extraction/claudeCodeRunner.ts` and `src/extraction/prompts/extractFacts.ts`, with their tests in `tests/extraction/claudeCodeRunner.test.ts` and `tests/extraction/extractFacts.test.ts` updated to match — committed as `0dcf085` after Task 5, verified passing (68/68) and re-confirmed against the real CLI producing correctly-categorized, appropriately-varied-confidence facts on real content.

**2a. Real gap found during execution (not caught in self-review):** Task 4 missed that the base plan's `tests/cli/integration.test.ts` calls `runExtract(root, path, { actor })` with no runner override — once the default flipped to `claudeCodeRunner`, that test started spawning a real `claude` process and failing. Fixed by adding `stub: true` to that call. Recorded here so a future reader knows why that line looks different from the base plan's original text.
