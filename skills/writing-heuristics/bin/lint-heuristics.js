...
var VFile = class {
  ...
  set basename(basename2) {
    ...
    this.path = import_node_path.default.join(this.dirname || "", basename2);
  }
  /**
...
function parseMarkdown(content3) {
  const processor = unified().use(remarkParse).use(remarkFrontmatter, ["yaml", "toml"]).use(remarkGfm);
  const ast = processor.parse(content3);
  const proseNodes = [];
  const linkNodes = [];
  const comments = [];
  visit(ast, (node2) => {
    if (node2.type === "code" || node2.type === "inlineCode" || node2.type === "table" || node2.type === "tableRow" || node2.type === "tableCell" || node2.type === "yaml" || node2.type === "toml") {
      return;
    }
    if (node2.type === "html") {
      const match = node2.value.match(/^<!--\s*(.*?)\s*-->$/s);
      if (match) {
        comments.push({
          value: match[1].trim(),
          line: node2.position?.start?.line ?? 1,
          column: node2.position?.start?.column ?? 1
        });
      }
      return;
    }
    if (node2.type === "link") {
      let linkText = "";
      if (node2.children) {
        linkText = node2.children.map((c) => c.value ?? "").join("");
      }
      linkNodes.push({
        text: linkText.trim(),
        url: node2.url ?? "",
        line: node2.position?.start?.line ?? 1,
        column: node2.position?.start?.column ?? 1,
        endLine: node2.position?.end?.line ?? 1,
        endColumn: node2.position?.end?.column ?? 1
      });
      return;
    }
...
function lintText(content3, options = {}) {
  const catalog = getCatalog();
  const parsed = parseMarkdown(content3);
  ...
  for (const node2 of parsed.proseNodes) {
    for (const rule of catalog.rules) {
      if (!options.noSuppress && suppressions.isSuppressed(rule.id, node2.line)) {
        continue;
      }
      if (rule.id === "ban-throat-clearing" && node2.type === "paragraph") {
        const regex = /^(Certainly|Sure thing|Sure|Here is|In this section|Let's dive|Note that|Allow me to|Of course)[!,.:]?\s*/i;
        const m = node2.text.match(regex);
        if (m) {
          violations.push({
            ruleId: rule.id,
            ruleName: rule.name,
      ...
      if (rule.id === "ban-filler-adverbs") {
        ...
        while ((match = regex.exec(node2.text)) !== null) {
          violations.push({
            ...
            line: node2.line,
            column: node2.column + match.index,
            autofix: false,
            confidence: rule.confidence
          });
        }
      ...
      if (rule.id === "heading-sentence-case" && node2.type === "heading") {
        const words = node2.text.split(/\s+/);
        ...
        if (titleCaseCount >= 2) {
          violations.push({
            ...
            ruleName: rule.name,
            severity: rule.severity,
            message: `Heading must be Sentence case: "${node2.text}".`,
            line: node2.line,
            column: node2.column,
            autofix: true,
            confidence: rule.confidence
  ...
  for (const link2 of parsed.linkNodes) {
    if (!options.noSuppress && suppressions.isSuppressed("descriptive-links", link2.line)) {
      continue;
    }
    const genericLinkRegex = /^(here|click here|link|this link|this page|read more|more|more info|website)$/i;
    if (genericLinkRegex.test(link2.text)) {
      violations.push({
        ruleId: "descriptive-links",
        ruleName: "Descriptive Link Text",
        severity: "error",
        message: `Generic link text "${link2.text}" is not descriptive. Name the specific destination.`,
        line: link2.line,
        column: link2.column,
        autofix: false,
        confidence: 1
...
async function lintFile(filePath, options = {}) {
  ...
  return lintText(content3, { ...options, filePath });
}
...
function toSentenceCase(headingText) {
  ...
  return words.map((w, index2) => {
    if (index2 === 0 || /\s+/.test(w) || /^[A-Z0-9_]{2,}$/.test(w) || /^`.*`$/.test(w)) {
      return w;
    }
    return w.toLowerCase();
  }).join("");
}
function applyFixes(content3, violations) {
  const eol = content3.includes("\r\n") ? "\r\n" : "\n";
  const hasBom = content3.charCodeAt(0) === 65279;
  let text5 = hasBom ? content3.slice(1) : content3;
  let appliedCount = 0;
...
async function safeFixFile(filePath, options = {}) {
  const content3 = fs2.readFileSync(filePath, "utf8");
  const lintRes = lintText(content3, { filePath });
  const fixRes = applyFixes(content3, lintRes.violations);
  ...
  const tmpPath = path2.join(dir, `${base}.${pid}.${timestamp}.tmp`);
  let backupPath;
  if (!options.noBackup) {
    let bakSuffix = "";
    let counter = 0;
    while (fs2.existsSync(path2.join(dir, `${base}.${timestamp}.bak${bakSuffix}`))) {
      counter++;
      bakSuffix = `.${counter}`;
    }
    backupPath = path2.join(dir, `${base}.${timestamp}.bak${bakSuffix}`);
  }
  try {
...
`;
    for (const v of res.violations) {
      const color2 = v.severity === "error" ? "\x1B[31merror\x1B[0m" : "\x1B[33mwarning\x1B[0m";
      output += `  ${v.line}:${v.column}  ${color2}  ${v.message}  \x1B[90m${v.ruleId}\x1B[0m
`;
