...
import { HeuristicsCatalog, RuleDefinition } from './types';

export function compileCatalog(catalogPath: string): { skillMd: string; rulesMd: string } {
  const raw = fs.readFileSync(catalogPath, 'utf8');
  const catalog: HeuristicsCatalog = JSON.parse(raw);
