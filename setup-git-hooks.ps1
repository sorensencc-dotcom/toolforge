...
"@
    ...
    $preCommitShim = @"
#!/usr/bin/env bash
# Installed by setup-git-hooks.ps1
# Chains governance validation and secret scanning with the retro/roadmap/CI
# gate below. Order-independent: whichever of setup-git-hooks.ps1 /
# setup-git-hook.mjs runs last writes this same chained shim, so neither
# installer can silently drop the other's checks.
