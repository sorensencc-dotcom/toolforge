...
"@
    ...
    $canonicalShimPath = Join-Path $Repo 'CIC-GOVERNANCE\scripts\pre-commit-shim.sh'
    if (-not (Test-Path $canonicalShimPath)) {
        Write-Error "Canonical pre-commit shim not found: $canonicalShimPath"
        exit 1
    }
    $preCommitShim = Get-Content -Raw $canonicalShimPath
