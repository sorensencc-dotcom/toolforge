...
# and failures (e.g. unsupported .heic) are visible without rerunning everything.
set -uo pipefail
...
already_ingested() {
  ...
  node -e '
    const fs = require("fs");
    const path = "'"$VAULT"'/topics/charlie/willow-run/sources/metadata.json";
...
total=0 ok=0 skipped=0 failed=0 unsupported=0
LIMIT="${LIMIT:-0}"  # 0 = no limit; set LIMIT=N to smoke-test on N files
...
while IFS= read -r -d '' file; do
  ...
  if [ "$ext_lower" = "heic" ]; then
    ...
    echo "UNSUPPORTED (heic, no converter): $basename_file" | tee -a "$LOG"
    continue
  fi
...
echo "total: $total  ok: $ok  skipped: $skipped  failed: $failed  unsupported(heic): $unsupported" | tee -a "$LOG"
echo "log: $LOG"
