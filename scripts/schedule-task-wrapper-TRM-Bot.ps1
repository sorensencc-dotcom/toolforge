...
<#
.SYNOPSIS
Windows Scheduled Task wrapper for TRM Gap Triage & RFC Drafter Bot under \Ironbots\ category.

.DESCRIPTION
Automates periodic execution of scripts/trm-bot-runner.mjs to triage
research gaps, query topic context, and draft RFC decision notes.
...
$scriptPath = Join-Path $RepoRoot 'scripts\trm-bot-runner.mjs'
$stdoutLog = Join-Path $LogDirectory 'trm-bot.stdout.log'
$stderrLog = Join-Path $LogDirectory 'trm-bot.stderr.log'
...
switch ($Action) {
    'Register' {
        if (-not (Test-Path -LiteralPath $scriptPath -PathType Leaf)) {
            throw "TRM bot script does not exist: $scriptPath"
        }
        ...
        if ($isAdmin -or $Unattended) {
            try {
                ...
                Register-ScheduledTask `
                    ...
                    -Settings $settings `
                    -Principal $principal `
                    -Description 'Ironbots: Periodic TRM research gap triage and RFC note drafting (Unattended).' `
                ...
                Register-ScheduledTask `
                    ...
                    -Trigger $trigger `
                    -Settings $settings `
                    -Description 'Ironbots: Periodic TRM research gap triage and RFC note drafting.' `
                    -Force:$Force | Out-Null
    ...
    'Test' {
        ...
        Write-Host "[Test] Executing TRM bot directly..." -ForegroundColor Cyan
        & $NodePath $scriptPath --dry-run "--limit=$Limit"
    }
}
