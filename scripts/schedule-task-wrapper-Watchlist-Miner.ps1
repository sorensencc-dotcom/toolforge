...
<#
.SYNOPSIS
Windows Scheduled Task wrapper for Watchlist & Competitor Drift Miner Bot under \Ironbots\ category.

.DESCRIPTION
Automates periodic execution of scripts/watchlist-miner-bot.mjs to monitor
upstream model architecture baselines and track competitor drifts.
...
$scriptPath = Join-Path $RepoRoot 'scripts\watchlist-miner-bot.mjs'
$stdoutLog = Join-Path $LogDirectory 'watchlist-miner-bot.stdout.log'
$stderrLog = Join-Path $LogDirectory 'watchlist-miner-bot.stderr.log'
...
switch ($Action) {
    'Register' {
        if (-not (Test-Path -LiteralPath $scriptPath -PathType Leaf)) {
            throw "Watchlist Miner script does not exist: $scriptPath"
        }
        ...
        if ($isAdmin -or $Unattended) {
            try {
                ...
                Register-ScheduledTask `
                    ...
                    -Settings $settings `
                    -Principal $principal `
                    -Description 'Ironbots: Periodic Competitor & Model Watchlist Drift Miner (Unattended).' `
                ...
                Register-ScheduledTask `
                    ...
                    -Trigger $trigger `
                    -Settings $settings `
                    -Description 'Ironbots: Periodic Competitor & Model Watchlist Drift Miner.' `
                    -Force:$Force | Out-Null
    ...
    'Test' {
        ...
        Write-Host "[Test] Executing Watchlist-Miner bot directly..." -ForegroundColor Cyan
