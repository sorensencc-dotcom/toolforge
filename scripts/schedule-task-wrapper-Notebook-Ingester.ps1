...
<#
.SYNOPSIS
Windows Scheduled Task wrapper for NotebookLM & Knowledge Ingester Bot under \Ironbots\ category.

.DESCRIPTION
Automates periodic execution of scripts/notebook-ingester-bot.mjs to maintain
the local SQLite FTS5 knowledge search index (.kb_cache/knowledge_fts5.db).
...
param(
    ...
    [string]$TaskName = 'Notebook-Ingester',
...
$stdoutLog = Join-Path $LogDirectory 'notebook-ingester-bot.stdout.log'
$stderrLog = Join-Path $LogDirectory 'notebook-ingester-bot.stderr.log'
...
switch ($Action) {
    'Register' {
        if (-not (Test-Path -LiteralPath $scriptPath -PathType Leaf)) {
            throw "Ingester script does not exist: $scriptPath"
        }
        ...
        if ($isAdmin -or $Unattended) {
            try {
                ...
                Register-ScheduledTask `
                    ...
                    -Settings $settings `
                    -Principal $principal `
                    -Description 'Ironbots: Periodic Knowledge Ingestion and SQLite FTS5 Indexer (Unattended).' `
                ...
                Register-ScheduledTask `
                    ...
                    -Trigger $trigger `
                    -Settings $settings `
                    -Description 'Ironbots: Periodic Knowledge Ingestion and SQLite FTS5 Indexer.' `
                    -Force:$Force | Out-Null
    ...
    'Test' {
        ...
        Write-Host "[Test] Executing Notebook-Ingester bot directly..." -ForegroundColor Cyan
