...
import { createBrowserAdapter, normalizeBrowserObservation, pageEvidenceExpression } from '../browser-adapter.mjs';

const nodeExecutable = process.execPath;

test('scopes Neo page evidence to GitHub wiki content', () => {
  const expression = pageEvidenceExpression([{ selector: '.diagram-container > svg', sourceAsset: 'wiki/page.html' }]);
  assert.match(expression, /#wiki-body, \.wiki-wrapper \.markdown-body, #wiki-content, \.markdown-body/);
  assert.match(expression, /contentRoot\.querySelectorAll\('a\[href\]'\)/);
  assert.match(expression, /contentRoot\.querySelectorAll\(rule\.selector\)/);
});
...
test('constructs safe gstack command for page navigation', async () => {
  ...
  assert.equal(calls.filter(({ args }) => args[0] === 'js').every(({ args }) => /contentRoot\.querySelectorAll\('img'\)/.test(args[1])), true);
