...
export function pageEvidenceExpression(diagramRules = []) {
  ...
  return `(async () => {
    ...
    const captionFor = (element) => {
      ...
      if (caption || heading) return (caption || heading).trim();
      const root = element.closest('#wiki-body, #wiki-content, .markdown-body') || document.body;
      const headings = [...root.querySelectorAll('h1,h2,h3,h4,h5,h6')];
      const previous = headings.filter((candidate) => candidate.compareDocumentPosition(element) & Node.DOCUMENT_POSITION_FOLLOWING).pop();
    ...
    for (const anchor of contentRoot.querySelectorAll('a[href]')) {
      ...
      seenHref.add(href);
      anchorEntries.push({ text, href, samePage: rawHref.startsWith('#') });
      if (anchorEntries.length >= 400) break;
    }
    const links = await Promise.all(anchorEntries.map(async ({ text, href, samePage }) => {
      if (samePage) return { text, href, inScope: true, ok: true, status: 200 };
      const url = new URL(href);
      const inScope = url.origin === document.location.origin;
      if (!inScope) return { text, href, inScope, ok: false, status: null };
      try {
        const response = await fetch(url.href, { credentials: 'same-origin', redirect: 'follow', signal: AbortSignal.timeout(8000) });
    ...
    const diagrams = ${JSON.stringify(rules)}.flatMap((rule) => {
      let elements;
      try { elements = [...contentRoot.querySelectorAll(rule.selector)]; } catch { return []; }
    ...
    return JSON.stringify({
      images: [...contentRoot.querySelectorAll('img')].slice(0, 200).map(imageFor),
      diagrams,
